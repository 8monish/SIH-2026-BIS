...
import { getActiveGeminiApiKey, saveActiveGeminiApiKey, getActiveGeminiModel, saveActiveGeminiModel, callLiveGeminiAPI, buildBISSystemPrompt } from './gemini-bridge.js';
...
export function initManakBotPage() {
  ...
  async function handleStudioFileUpload(file, userNote = '', cachedDataUrl = null) {
    ...
    const processData = async (base64Data) => {
      ...
      // Pass file directly to Gemini — no OCR, no preprocessing
      const apiKey = getActiveGeminiApiKey();
      if (!apiKey) {
        removeStudioTyping();
        appendStudioMessage(
          `📎 **${file.name}** received.\n\nTo analyze this file, please add a **Gemini API key** in the sidebar. Get a free key at [Google AI Studio](https://aistudio.google.com).`,
          'bot'
        );
        return;
      }
      ...
      // User's typed message is the prompt; if nothing typed, analyze for BIS compliance
        ...
        try {
          ...
          if (response.ok) {
            ...
            if (replyText && replyText.trim()) {
              removeStudioTyping();
              appendStudioMessage(replyText.trim(), 'bot');
          ...
          removeStudioTyping();
          appendStudioMessage(`⚠️ Gemini error (${response.status}): ${errData?.error?.message || response.statusText}. Please check your API key.`, 'bot');
          return;

        } catch (err) {
          if (err.name === 'AbortError') continue;
          removeStudioTyping();
          appendStudioMessage(`⚠️ Network error: ${err.message || 'Could not reach Gemini API'}.`, 'bot');
          return;
        }
      }

      removeStudioTyping();
      appendStudioMessage('⚠️ None of the available Gemini models could process this file. Try selecting a different model in the sidebar.', 'bot');
    };
